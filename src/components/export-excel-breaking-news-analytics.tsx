...
export const ExportExcelBreakingNewsAnalytics = ({
  filter
}: {
  ...
  const handleExportExcelNews = async () => {
    ...
    const response = await defaultApiFetcher.get('news/breaking-news-analytics', {
      query: {
        page: 1,
  ...
  const handleExport = async (news: any[]) => {
    if (news.length > 0) {
      ...
      const worksheet = workbook.addWorksheet('Breaking News Analytics');
